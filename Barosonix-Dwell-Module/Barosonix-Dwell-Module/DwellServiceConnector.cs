...
{
    public class DwellServiceConnector
    {
            ...
            try {
                ...
                if (hashtable3.ContainsKey ("success")) {
                    if ((string)hashtable3 ["success"] == "true") {
                        ...
                        if (debug) {
                            DwellServiceConnector.m_log.DebugFormat ("[Web Store Debug] Success", new object[0]);
                        }
                    } else {
                        ...
                        if (debug) {
                            DwellServiceConnector.m_log.DebugFormat ("[Web Store Debug] Fail", new object[0]);
                        }
                    }
                } else {
                    hashtable.Add ("success", "false");
                    hashtable.Add ("result", "The region server did not respond!");
                    DwellServiceConnector.m_log.DebugFormat ("[Web Store Robust Module]: No response from Region Server! {0}", url);
                }
            } catch (WebException ex) {
                hashtable.Add ("success", "false");
                DwellServiceConnector.m_log.ErrorFormat ("[STORE]: Error sending transaction to {0} the host didn't respond " + ex.ToString (), url);
            }
            return hashtable;
        }
